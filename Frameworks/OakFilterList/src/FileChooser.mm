...
static NSButton* CreateScopeButton (NSString* label, SEL action, NSUInteger tag)
{
...
- (id)init
{
	if((self = [super init]))
	{
		...
		_allButton           = CreateScopeButton(@"All",                   @selector(takeSourceIndexFrom:), 0);
		_openDocumentsButton = CreateScopeButton(@"Open Documents",        @selector(takeSourceIndexFrom:), 1);
		_scmChangesButton    = CreateScopeButton(@"Uncommitted Documents", @selector(takeSourceIndexFrom:), 2);
		...
		[contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(8)-[searchField]-(4)-[aboveScopeBarDark(==1)][aboveScopeBarLight(==1)]-(4)-[allButton]-(4)-[topDivider(==1)][scrollView(>=50)][bottomDivider(==1)]-[statusTextField]-(8)-|" options:0 metrics:nil views:views]];
...
- (void)updateGoToMenu:(NSMenu*)aMenu
{
	...
	{
		[[aMenu addItemWithTitle:@"Project Folder" action:@selector(takeSourceIndexFrom:) keyEquivalent:@"1"] setTag:0];
		[[aMenu addItemWithTitle:@"Open Documents" action:@selector(takeSourceIndexFrom:) keyEquivalent:@"2"] setTag:1];
		[[aMenu addItemWithTitle:@"Uncommitted Documents" action:@selector(takeSourceIndexFrom:) keyEquivalent:@"3"] setTag:2];
	}
	else
	{
